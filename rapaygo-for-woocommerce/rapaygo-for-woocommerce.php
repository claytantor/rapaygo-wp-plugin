...
//   }
// } else {
//     throw new \Exception('The Rapaygo payment plugin was not installed correctly or the files are corrupt. Please reinstall the plugin. If this message persists after a reinstall, contact the Rapaygo team through https://chat.rapaygoserver.org with this message.');
// }
...
function woocommerce_rapaygo_init()
{
    ...
    class WC_Gateway_Rapaygo extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            // $this->api_key            = get_option('woocommerce_rapaygo_key');
            // $this->api_pub            = get_option('woocommerce_rapaygo_pub');
            // $this->api_sin            = get_option('woocommerce_rapaygo_sin');
            // $this->api_token          = get_option('woocommerce_rapaygo_token');
            // $this->api_token_label    = get_option('woocommerce_rapaygo_label');
            // $this->api_url        = get_option('woocommerce_rapaygo_url');

			// define rapaygo settings
			$this->api_key            = $this->get_option('woocommerce_rapaygo_key');
			$this->api_secret            = $this->get_option('woocommerce_rapaygo_secret');
			// $this->api_sin            = $this->get_option('woocommerce_rapaygo_sin');
			// $this->api_token          = $this->get_option('woocommerce_rapaygo_token');
			// $this->api_token_label    = $this->get_option('woocommerce_rapaygo_label');
			$this->api_url           = $this->get_option('woocommerce_rapaygo_url');
            ...
            //     try {
            //         $this->api_key    = $this->rapaygo_decrypt($this->api_key);
            ...
            // if (false === empty($this->api_secret)) {
            //     try {
            //         $this->api_pub    = $this->rapaygo_decrypt($this->api_pub);
		...
		// 				$classcode = "class {$token['classname']} extends WC_Gateway_Rapaygo { ";
		...
		// 				$classcode .= "\$this->id = 'rapaygo_{$token['symbol']}';";
		// 				$classcode .= "\$this->method_title = 'Rapaygo Asset: {$token['symbol']}';";
		// 				$classcode .= "\$this->method_description = 'This is an additional asset managed by Rapaygo.';";
        ...
        public function is_valid_for_use()
        {
            ...
            //         $this->log('    [Error] The Rapaygo payment plugin was called to check if it was valid for use but could not instantiate a currency object.');
            //         throw new \Exception('The Rapaygo payment plugin was called to check if it was valid for use but could not instantiate a currency object. Cannot continue!');
            //     }
            // } catch (\Exception $e) {
        ...
        public function init_form_fields()
        {
            ...
            $this->form_fields = array(
                'title' => array(
                    ...
                    'default'     => __('Bitcoin', 'rapaygo-for-woocommerce'),
                    'desc_tip'    => true,
               ),
               'description' => array(
            ...
            //    ),
               'transaction_speed' => array(
                    'title'       => __('Invoice pass to "confirmed" state after', 'rapaygo-for-woocommerce'),
                    'type'        => 'select',
               ...
               'notification_url' => array(
                    'title'       => __('Notification URL', 'rapaygo-for-woocommerce'),
                    'type'        => 'url',
                    'description' => __('Rapaygo will send IPNs for orders to this URL with the Rapaygo invoice data', 'rapaygo-for-woocommerce'),
                    'default'     => '',
                    'placeholder' => WC()->api_request_url('WC_Gateway_Rapaygo'),
                    'desc_tip'    => true,
               ),
               'redirect_url' => array(
                    'title'       => __('Redirect URL', 'rapaygo-for-woocommerce'),
                    'type'        => 'url',
                    'description' => __('After paying the Rapaygo invoice, users will be redirected back to this URL', 'rapaygo-for-woocommerce'),
                    'default'     => '',
                    'placeholder' => $this->get_return_url(),
                    'desc_tip'    => true,
               ),
            //    'additional_tokens' => array(
            //       'title'       => __('Additional token configuration', 'rapaygo-for-woocommerce'),
            //       'type'        => 'textarea',
            //       'description' => __('You can configure additional tokens here, one per line. e.g. "HAT;Hat Token;promotion" See documentation for details. Each one will be available as their own payment method.', 'rapaygo-for-woocommerce'),
            //       'default'     => '',
            //       'desc_tip'    => true,
            //    ),
			//    'additional_tokens_limit_payment' => array(
			// 		'title'       => __('Additional tokens: Enforce payment tokens', 'rapaygo-for-woocommerce'),
			// 		'type'        => 'checkbox',
			// 		'label'       => __('Limit default payment methods to listed "payment" tokens.', 'rapaygo-for-woocommerce'),
			// 		'default'     => 'no',
			// 		'value'       => 'yes',
			// 		'description' => __('This will override the default rapaygo payment method (defaults to all supported by Rapaygo Server) and enforce to tokens of type "payment". This is useful if you want full control on what is available on Rapaygo Server payment page.', 'rapaygo-for-woocommerce'),
			// 		'desc_tip'    => true,
			//    ),
        ...
        public function generate_api_token_html()
        {
            ...
            wp_enqueue_style('rapaygo-token', plugins_url('assets/css/style.css', __FILE__));
            wp_enqueue_script('rapaygo-pairing', plugins_url('assets/js/pairing.js', __FILE__), array('jquery'), null, true);
            wp_localize_script( 'rapaygo-pairing', 'RapaygoAjax', array(
                'ajaxurl'     => admin_url( 'admin-ajax.php' ),
                'pairNonce'   => wp_create_nonce( 'rapaygo-pair-nonce' ),
                'revokeNonce' => wp_create_nonce( 'rapaygo-revoke-nonce' )
                )
            );
            ...
            <tr valign="top">
                ...
                <td class="forminp" id="rapaygo_api_token">
                    <div id="rapaygo_api_token_form">
                        <?php
        ...
        public function process_payment($order_id)
        {
            ...
            $notification_url = $this->get_option('notification_url', WC()->api_request_url('WC_Gateway_Rapaygo'));
            ...
            // if (false === isset($currency) && true === empty($currency)) {
            //     $this->log('    [Error] The Rapaygo payment plugin was called to process a payment but could not instantiate a Currency object.');
            //     throw new \Exception('The Rapaygo payment plugin was called to process a payment but could not instantiate a Currency object. Cannot continue!');
            // }
            ...
            //     $this->log('    [Error] The Rapaygo payment plugin was called to process a payment but could not instantiate a CurlAdapter object.');
            //     throw new \Exception('The Rapaygo payment plugin was called to process a payment but could not instantiate a CurlAdapter object. Cannot continue!');
            // }
            ...
            // } else {
            //     $this->log('    [Error] The Rapaygo payment plugin was called to process a payment but could not set client->setPrivateKey to this->api_key. The empty() check failed!');
            //     throw new \Exception(' The Rapaygo payment plugin was called to process a payment but could not set client->setPrivateKey to this->api_key. The empty() check failed!');
            // }

            // if (false === empty($this->api_pub)) {
            //     $client->setPublicKey($this->api_pub);
            // } else {
            //     $this->log('    [Error] The Rapaygo payment plugin was called to process a payment but could not set client->setPublicKey to this->api_pub. The empty() check failed!');
            //     throw new \Exception(' The Rapaygo payment plugin was called to process a payment but could not set client->setPublicKey to this->api_pub. The empty() check failed!');
            // }

            // if (false === empty($this->api_token)) {
            //     $client->setToken($this->api_token);
            // } else {
            //     $this->log('    [Error] The Rapaygo payment plugin was called to process a payment but could not set client->setToken to this->api_token. The empty() check failed!');
            //     throw new \Exception(' The Rapaygo payment plugin was called to process a payment but could not set client->setToken to this->api_token. The empty() check failed!');
            // }
        ...
        public function ipn_callback()
        {
            ...
            if (true === empty($post)) {
                ...
                error_log('[Error] Rapaygo plugin received empty POST data for an IPN message.');
            ...
            if (true === empty($json)) {
                ...
                error_log('[Error] Rapaygo plugin received an invalid JSON payload sent to IPN handler: ' . $post);
            ...
            if (false === array_key_exists('id', $json)) {
                ...
                error_log('[Error] Rapaygo plugin did not receive an invoice ID present in JSON payload: ' . var_export($json, true));
            ...
            if (false === array_key_exists('url', $json)) {
                ...
                error_log('[Error] Rapaygo plugin did not receive an invoice URL present in JSON payload: ' . var_export($json, true));
            ...
            if (false === isset($client) && true === empty($client)) {
                $this->log('    [Error] The Rapaygo payment plugin was called to handle an IPN but could not instantiate a client object.');
                throw new \Exception('The Rapaygo payment plugin was called to handle an IPN but could not instantiate a client object. Cannot continue!');
            } else {
            ...
            if (false === isset($curlAdapter) && true === empty($curlAdapter)) {
                $this->log('    [Error] The Rapaygo payment plugin was called to handle an IPN but could not instantiate a CurlAdapter object.');
                throw new \Exception('The Rapaygo payment plugin was called to handle an IPN but could not instantiate a CurlAdapter object. Cannot continue!');
            } else {
            ...
            if (false === empty($this->api_key)) {
                ...
                $this->log('    [Error] The Rapaygo payment plugin was called to handle an IPN but could not set client->setPrivateKey to this->api_key. The empty() check failed!');
                throw new \Exception('The Rapaygo payment plugin was called to handle an IPN but could not set client->setPrivateKey to this->api_key. The empty() check failed!');
            }

            if (false === empty($this->api_pub)) {
                $client->setPublicKey($this->api_pub);
            } else {
                $this->log('    [Error] The Rapaygo payment plugin was called to handle an IPN but could not set client->setPublicKey to this->api_pub. The empty() check failed!');
                throw new \Exception('The Rapaygo payment plugin was called to handle an IPN but could not set client->setPublicKey to this->api_pub. The empty() check failed!');
            }

            if (false === empty($this->api_token)) {
                $client->setToken($this->api_token);
            } else {
                $this->log('    [Error] The Rapaygo payment plugin was called to handle an IPN but could not set client->setToken to this->api_token. The empty() check failed!');
                throw new \Exception('The Rapaygo payment plugin was called to handle an IPN but could not set client->setToken to this->api_token. The empty() check failed!');
            }

            $this->log('    [Info] Key and token empty checks passed.  Parameters in client set accordingly...');

            // Fetch the invoice from Rapaygo Server to update the order
            try {
            ...
            // The Rapaygo order id is the WooCommerce order number, see $invoice->setOrderId in process_payment()
            $order_number = $invoice->getOrderId();

            // Get the actual WooCommerce ID from the meta data, via the Rapaygo invoice id.
            ...
            if (count($order_query) === 1) {
                ...
                $this->log('    [Error] Can\'t find order by Rapaygo meta data. Using order ID fallback from posData.');
            }

            $responseData = json_decode($client->getResponse()->getBody());

            if (false === isset($order_id) && true === empty($order_id)) {
                $this->log('    [Error] The Rapaygo payment plugin was called to process an IPN message but could not obtain the order ID from the invoice.');
                throw new \Exception('The Rapaygo payment plugin was called to process an IPN message but could not obtain the order ID from the invoice. Cannot continue!');
            } else {
            ...
            if (false === $order) {
                $this->log('    [Error] The Rapaygo payment plugin was called to process an IPN message but could not retrieve the order details for order_id: "' . $order_id . '". If you use an alternative order numbering system, please see class-wc-gateway-rapaygo.php to apply a search filter.');
                throw new \Exception('The Rapaygo payment plugin was called to process an IPN message but could not retrieve the order details for order_id ' . $order_id . '. Cannot continue!');
            } else {
                $this->log('    [Info] Order details retrieved successfully...');
            }

            if(!$this->is_rapaygo_payment_method($order))
            {
                $this->log('    [Info] Not using rapaygo payment method...');
                $this->log('    [Info] Leaving ipn_callback()...');
                return;
            }

            $expected_invoiceId = get_post_meta($order_id, 'Rapaygo_id', true);
            ...
            {
                $this->log('    [Error] Received IPN for order '. $order_id . ' with Rapaygo invoice id ' . $json['id'] . ' while expected Rapaygo invoice is ' . $expected_invoiceId);
                throw new \Exception('Received IPN for order '. $order_id . ' with Rapaygo invoice id ' . $json['id'] . ' while expected Rapaygo invoice is ' . $expected_invoiceId);
            }

            $current_status = $order->get_status();

            if (false === isset($current_status) || true === empty($current_status)) {
                $this->log('    [Error] The Rapaygo payment plugin was called to process an IPN message but could not obtain the current status from the order.');
                throw new \Exception('The Rapaygo payment plugin was called to process an IPN message but could not obtain the current status from the order. Cannot continue!');
            } else {
            ...
            if (false === isset($checkStatus) && true === empty($checkStatus)) {
                $this->log('    [Error] The Rapaygo payment plugin was called to process an IPN message but could not obtain the current status from the invoice.');
                throw new \Exception('The Rapaygo payment plugin was called to process an IPN message but could not obtain the current status from the invoice. Cannot continue!');
            } else {
            ...
            if($event === "")
            {
                switch ($checkStatus) {
                    ...
                    case 'paid':
                        ...
                        if($paid_status !== 'RAPAYGO_IGNORE')
                            $order->update_status($paid_status);
                        $order->add_order_note(__('Rapaygo invoice paid. Awaiting network confirmation and payment completed status.', 'rapaygo-for-woocommerce'));
                        break;
                    ...
                    case 'confirmed':
                        ...
                        if($confirmed_status !== 'RAPAYGO_IGNORE')
                            $order->update_status($confirmed_status);
                        $order->add_order_note(__('Rapaygo invoice confirmed. Awaiting payment completed status.', 'rapaygo-for-woocommerce'));
                        break;
                    ...
                    case 'complete':
                        ...
                        if($complete_status !== 'RAPAYGO_IGNORE')
                            $order->update_status($complete_status);
                        $order->add_order_note(__('Rapaygo invoice payment completed. Payment credited to your merchant account.', 'rapaygo-for-woocommerce'));
                        break;
                    ...
                    case 'invalid':
                        ...
                        if($invalid_status !== 'RAPAYGO_IGNORE')
                            $order->update_status($invalid_status, __('Bitcoin payment is invalid for this order! The payment was not confirmed by the network within on time. Do not ship the product for this order!', 'rapaygo-for-woocommerce'));
                        break;

                    case 'expired':

                        $this->log('    [Info] The invoice is in the "expired" status...');
                        if($expired_status !== 'RAPAYGO_IGNORE')
                            $order->update_status($expired_status, __('Bitcoin payment has expired for this order! The payment was not broadcasted before its expiration. Do not ship the product for this order!', 'rapaygo-for-woocommerce'));
                        break;
            ...
            else //  is an event
            {
                ...
                if ($event['code'] === 1009)
                {
                    ...
                    if($event_invoice_paidAfterExpiration !== 'RAPAYGO_IGNORE')
                        $order->update_status($event_invoice_paidAfterExpiration , __('A payment has arrived late for this order!', 'rapaygo-for-woocommerce'));
                    $order->add_order_note(__('A payment has been received after expiration', 'rapaygo-for-woocommerce'));
                }
                if ($event['code'] === 2000)
                {
                    $this->log('    [Info] The invoice has expired while a partial payment has been sent...');
                    if($event_invoice_expiredPaidPartial !== 'RAPAYGO_IGNORE')
                        $order->update_status($event_invoice_expiredPaidPartial , __('The invoice has expired while a partial payment has been sent'));
                    $order->add_order_note(__('The invoice has expired while a partial payment has been sent', 'rapaygo-for-woocommerce'));
                }
            }
            $this->log('    [Info] Leaving ipn_callback()...');
        }

        public function update_rapaygo($order_id, $responseData)
        {
            update_post_meta($order_id, 'Rapaygo_btcPrice', $responseData->data->btcPrice);
            update_post_meta($order_id, 'Rapaygo_btcPaid', $responseData->data->btcPaid);
            update_post_meta($order_id, 'Rapaygo_BTCaddress', $responseData->data->bitcoinAddress);
        }
        ...
        public function rapaygo_encrypt($data)
        {
            if (false === isset($data) || true === empty($data)) {
                throw new \Exception('The Rapaygo payment plugin was called to encrypt data but no data was passed!');
            }

            $this->log('    [Info] Entered rapaygo_encrypt...');
                ...
                strlen($fingerprint) > 24)
            {
                ...
                if (false === isset($fingerprint) || true === empty($fingerprint)) {
                    throw new \Exception('The Rapaygo payment plugin was called to encrypt data but could not generate a fingerprint parameter!');
                }

                $encrypted = $openssl_ext->encrypt(base64_encode(serialize($data)), $fingerprint, '1234567890123456');

                if (true === empty($encrypted)) {
                    throw new \Exception('The Rapaygo payment plugin was called to encrypt a serialized object and failed!');
                }

                $this->log('    [Info] Leaving class level rapaygo_encrypt...');

                return $encrypted;
            } else {
                $this->log('    [Error] Invalid server fingerprint generated in rapaygo_encrypt()');
                wp_die('Invalid server fingerprint generated');
            }
        }

        public function rapaygo_decrypt($encrypted)
        {
            if (false === isset($encrypted) || true === empty($encrypted)) {
                throw new \Exception('The Rapaygo payment plugin was called to decrypt data but no data was passed!');
            }

            $this->log('    [Info] Entered class level rapaygo_decrypt...');
                ...
                strlen($fingerprint) > 24)
            {
                ...
                if (false === isset($fingerprint) || true === empty($fingerprint)) {
                    throw new \Exception('The Rapaygo payment plugin was called to decrypt data but could not generate a fingerprint parameter!');
                }
                ...
                if (false === base64_decode($decrypted, true)) {
                    $this->log('    [Warning] In rapaygo_decrypt: data appears to have already been decrypted. Strict base64 check failed.');
                } else {
                    $decrypted = base64_decode($decrypted);
                }

                if (true === empty($decrypted)) {
                    throw new \Exception('The Rapaygo payment plugin was called to unserialize a decrypted object and failed! The decrypt function was called with "' . $encrypted . '"');
                }

                $this->log('    [Info] Leaving class level rapaygo_decrypt...');

                return unserialize($decrypted);
            } else {
                $this->log('    [Error] Invalid server fingerprint generated in rapaygo_decrypt()');
                wp_die('Invalid server fingerprint generated');
            }
   ...
   */
    ...
    // TODO: Try to find a way to make it work within the WC_Gateway_Rapaygo class
    add_action('wp_ajax_rapaygo_pair_code', 'ajax_rapaygo_pair_code');
    add_action('wp_ajax_rapaygo_revoke_token', 'ajax_rapaygo_revoke_token');
    add_action('wp_ajax_rapaygo_create_invoice', 'ajax_rapaygo_create_invoice');

    function ajax_rapaygo_pair_code()
    {
        $nonce = $_POST['pairNonce'];
        if ( ! wp_verify_nonce( $nonce, 'rapaygo-pair-nonce' ) ) {
            die ( 'Unauthorized!');
        }
        ...
        if ( current_user_can( 'manage_options' ) ) {
            ...
            if (true === empty($key)) {
                throw new \Exception('The Rapaygo payment plugin was called to process a pairing code but could not instantiate a PrivateKey object. Cannot continue!');
            }
            ...
            if (true === empty($pub)) {
                throw new \Exception('The Rapaygo payment plugin was called to process a pairing code but could not instantiate a PublicKey object. Cannot continue!');
            }
            ...
            if (true === empty($sin)) {
                throw new \Exception('The Rapaygo payment plugin was called to process a pairing code but could not instantiate a SinKey object. Cannot continue!');
            }
            ...
            if (true === empty($client)) {
                throw new \Exception('The Rapaygo payment plugin was called to process a pairing code but could not instantiate a Client object. Cannot continue!');
            }
            $client->setUri($url);
            $curlAdapter = new \Bitpay\Client\Adapter\CurlAdapter();

            if (true === empty($curlAdapter)) {
                throw new \Exception('The Rapaygo payment plugin was called to process a pairing code but could not instantiate a CurlAdapter object. Cannot continue!');
            }
            ...
            update_option('woocommerce_rapaygo_key', rapaygo_encrypt($key));
            update_option('woocommerce_rapaygo_pub', rapaygo_encrypt($pub));
            update_option('woocommerce_rapaygo_sin', (string)$sin);
            update_option('woocommerce_rapaygo_token', rapaygo_encrypt($token));
            update_option('woocommerce_rapaygo_label', $label);
            update_option('woocommerce_rapaygo_url', $url);
    ...
    function ajax_rapaygo_revoke_token()
    {
        $nonce = $_POST['revokeNonce'];
        if ( ! wp_verify_nonce( $nonce, 'rapaygo-revoke-nonce' ) ) {
            die ( 'Unauthorized!');
        }

        if ( current_user_can( 'manage_options' ) ) {
            update_option('woocommerce_rapaygo_key', null);
            update_option('woocommerce_rapaygo_pub', null);
            update_option('woocommerce_rapaygo_sin', null);
            update_option('woocommerce_rapaygo_token', null);
            update_option('woocommerce_rapaygo_label', null);
            update_option('woocommerce_rapaygo_network', 'testnet');
    ...
    function rapaygo_encrypt($data)
    {
            ...
            strlen($fingerprint) > 24)
        {
            ...
            if (true === empty($encrypted)) {
                throw new \Exception('The Rapaygo payment plugin was called to serialize an encrypted object and failed!');
            }
    ...
    function rapaygo_decrypt($encrypted)
    {
        if (false === isset($encrypted) || true === empty($encrypted)) {
            throw new \Exception('The Rapaygo payment plugin was called to decrypt data but no data was passed!');
        }
            ...
            strlen($fingerprint) > 24)
        {
            ...
            // Strict base64 char check
            if (false === base64_decode($decrypted, true)) {
                $error_string .= '    [Warning] In rapaygo_decrypt: data appears to have already been decrypted. Strict base64 check failed.';
            } else {
                $decrypted = base64_decode($decrypted);
            }
...
function woocommerce_rapaygo_activate()
{
    // Check for Requirements
    $failed = woocommerce_rapaygo_failed_requirements();
    ...
    if ($failed === false) {
        ...
        foreach ($plugins as $file => $plugin) {
            if ('Bitpay Woocommerce' === $plugin['Name'] && true === is_plugin_active($file)) {
                ...
                wp_die('Rapaygo for WooCommerce requires that the old plugin, <b>Bitpay Woocommerce</b>, is deactivated and deleted.<br><a href="'.$plugins_url.'">Return to plugins screen</a>');
            }
            if ('Rapaygo for WooCommerce' === $plugin['Name'] && true === is_plugin_active($file) && (0 > version_compare( $plugin['Version'], '3.0' ))) {
                deactivate_plugins(plugin_basename(__FILE__));
                wp_die('Rapaygo for WooCommerce requires that the 2.x version of this plugin is deactivated. <br><a href="'.$plugins_url.'">Return to plugins screen</a>');
            }
            if ('Rapaygo for WooCommerce' === $plugin['Name']
             && (0 > version_compare( $plugin['Version'], '3.0.1' ))) {
                update_option('woocommerce_rapaygo_key',
                    get_option( 'woocommerce_rapaygo_key', get_option('woocommerce_bitpay_key', null) ) );
                update_option('woocommerce_rapaygo_pub',
                get_option( 'woocommerce_rapaygo_pub', get_option('woocommerce_bitpay_pub', null) ) );
                update_option('woocommerce_rapaygo_sin',
                get_option( 'woocommerce_rapaygo_sin', get_option('woocommerce_bitpay_sin', null) ) );
                update_option('woocommerce_rapaygo_token',
                get_option( 'woocommerce_rapaygo_token', get_option('woocommerce_bitpay_token', null) ) );
                update_option('woocommerce_rapaygo_label',
                get_option( 'woocommerce_rapaygo_label', get_option('woocommerce_bitpay_label', null) ) );
                update_option('woocommerce_rapaygo_network',
                get_option( 'woocommerce_rapaygo_network', get_option('woocommerce_bitpay_network', null) ) );
                update_option('woocommerce_rapaygo_settings',
                get_option( 'woocommerce_rapaygo_settings', get_option('woocommerce_bitpay_settings', null) ) );
                update_option('woocommerce_rapaygo_url',
                get_option( 'woocommerce_rapaygo_url', get_option('woocommerce_bitpay_url', null) ) );
                update_option('woocommerce_rapaygo_notification_url',
                get_option( 'woocommerce_rapaygo_notification_url', get_option('woocommerce_bitpay_notification_url', null) ) );
                update_option('woocommerce_rapaygo_redirect_url',
                get_option( 'woocommerce_rapaygo_redirect_url', get_option('woocommerce_bitpay_redirect_url', null) ) );
                update_option('woocommerce_rapaygo_transaction_speed',
                get_option( 'woocommerce_rapaygo_transaction_speed', get_option('woocommerce_bitpay_transaction_speed', null) ) );
                update_option('woocommerce_rapaygo_order_states',
                get_option( 'woocommerce_rapaygo_order_states', get_option('woocommerce_bitpay_order_states', null) ) );

                set_transient( 'fx_admin_notice_show_migration_message', true, 5 );
            }
        }
        update_option('woocommerce_rapaygo_version', constant("RAPAYGO_VERSION"));
...
function fx_admin_notice_show_migration_message(){
    ...
    if( get_transient( 'fx_admin_notice_show_migration_message' ) ){
        ...
        <div class="notice notice-warning notice-alt is-dismissible">
            <p>The Rapaygo Plugin for Woocoomerce has been updated from a 2.x version!
            <strong>We have attempted to migrate your settings. Please double check them
            <?php echo '<a href="' . get_bloginfo('wpurl') . '/wp-admin/admin.php?page=wc-settings&tab=checkout&section=wc_gateway_rapaygo">here</a>'?>.
