...
function rapaygo_cart_init_handler()
{
    $orders_menu_permission = apply_filters('rapaygo_orders_menu_permission', 'manage_options');
    ...
    {
        rapaygo_create_orders_page();
    }
    else//Init hook handling code for front end
    {
        rapaygo_cart_actions_handler();
        add_filter('ngg_render_template','rapaygo_cart_ngg_template_handler',10,2);
        if(isset($_REQUEST['simple_cart_ipn']))
        {
            include_once('paypal.php');
            rapaygo_handle_paypal_ipn();
            exit;
        }
    }
    if (is_admin()) {
        add_action('admin_init', 'rapaygo_cart_add_tinymce_button');
	...
	if (isset($_GET['page']) && $_GET['page'] == 'wordpress-paypal-shopping-cart') {
	    ...
	    wp_redirect(get_admin_url() . 'admin.php?page=rapaygo-menu-main', 301);
	    exit;
	}
...
function rapaygo_cart_admin_init_handler()
{
    rapaygo_add_meta_boxes();
}

function rapaygo_number_format_price($price)
{
    $formatted_num = number_format($price,2,'.','');
    return $formatted_num;
}

function rapaygo_strip_char_from_price_amount($price_amount)
{
...
function rapaygo_append_values_to_custom_field($name,$value)
{
    $custom_field_val = $_SESSION['rapaygo_cart_custom_values'];
    $new_val = $name.'='.$value;
    ...
    $_SESSION['rapaygo_cart_custom_values'] = $custom_field_val;
    return $custom_field_val;
}

function rapaygo_cart_get_custom_var_array($custom_val_string)
{
    $delimiter = "&";
...
function rapaygo_cart_ngg_template_handler($arg1,$arg2)
{
    if($arg2=="gallery-wp-cart"){
        $template_name = "gallery-wp-cart";
        $gallery_template = RAPAYGO_CART_PATH. "/lib/$template_name.php";
        return $gallery_template;
    }
    return $arg1;
}

function rapaygo_insert_new_record()
{
...
function rapaygo_update_cart_items_record()
{
    if(isset($_SESSION['simpleCart']) && !empty($_SESSION['simpleCart']))
    {
...
function rapaygo_apply_dynamic_tags_on_email($text, $ipn_data, $args)
{
...
function rapaygo_run_activation()
{
    //General options
    add_option('rapaygo_cart_title', __("Your Shopping Cart", "rapaygo-wp-plugin"));
    add_option('rapaygo_cart_empty_text', __("Your cart is empty", "rapaygo-wp-plugin"));
    add_option('cart_return_from_paypal_url', get_bloginfo('wpurl'));

    //Add Confirmation Email Settings
    add_option("rapaygo_send_buyer_email", 1);
    $from_email_address = get_bloginfo('name')." <sales@your-domain.com>";
    add_option('rapaygo_buyer_from_email', $from_email_address);
    $buyer_email_subj = "Thank you for the purchase";
    add_option('rapaygo_buyer_email_subj', $buyer_email_subj);
    $email_body = "Dear {first_name} {last_name}"."\n";
    $email_body .= "\nThank you for your purchase! You ordered the following item(s):\n";
    $email_body .= "\n{product_details}";
    add_option('rapaygo_buyer_email_body', $email_body);

    $notify_email_address = get_bloginfo('admin_email');
    add_option('rapaygo_notify_email_address', $notify_email_address);
    $seller_email_subj = "Notification of product sale";
    add_option('rapaygo_seller_email_subj', $seller_email_subj);
    ...
    $seller_email_body .= "\n\nThanks";
    add_option('rapaygo_seller_email_body', $seller_email_body);

    //Generate and save a private key for this site
    $unique_id = uniqid('', true);
    add_option('rapaygo_private_key_one',$unique_id);
}

function rapaygo_settings_menu_footer()
{
    ?>
    <div class="rapaygo_yellow_box">
    <p>
        <?php _e("Need a shopping cart plugin with a lot of features and support? Check out our ", "rapaygo-wp-plugin"); ?>
        <a href="https://rapaygo.com" target="_blank"><?php _e("rapaygo Cart Plugin", "rapaygo-wp-plugin"); ?></a>
    </p>
    </div>
    <?php
}