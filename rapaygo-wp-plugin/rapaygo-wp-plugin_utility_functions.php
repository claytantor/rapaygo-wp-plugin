...
function rapaygo_log_payment_debug($message, $success, $end = false) {
    $logfile = RAPAYGO_CART_PATH . 'ipn_handle_debug.txt';
...
function rapaygo_reset_logfile()
{
    $log_reset = true;
    $logfile = RAPAYGO_CART_PATH . 'ipn_handle_debug.txt';
...
function rapaygo_get_total_cart_qty() {
    $total_items = 0;
...
function rapaygo_get_total_cart_sub_total() {
    $sub_total = 0;
    if (!isset($_SESSION['simpleCart'])) {
        return $sub_total;
    }
...
function rapaygo_clean_incomplete_old_cart_orders() {
...
function rapaygo_check_and_start_session() {
