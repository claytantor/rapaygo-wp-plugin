...
* Rapaygo WP Plugin
    ...
    @wordpress-plugin
    Plugin Name: Rapaygo Cart For WordPress
    Plugin URI: https://rapaygo.com
    description: A plugin to allow you to accept payments with Rapaygo
    Version: 1.2
    Author: Clay Graham
    Author URI: https://claytantor.com
    License: GPL2
    Text Domain: rapaygo-wp-cart
*/
...
define( 'RAPAYGO_CART_VERSION', '1.0.1' );
define( 'RAPAYGO_CART_FOLDER', dirname( plugin_basename( __FILE__ ) ) );

define( 'RAPAYGO_CART_PATH', plugin_dir_path( __FILE__ ) );
define( 'RAPAYGO_LANG_DIR', plugin_dir_path( __FILE__ ). "/languages" );
...
define( 'RAPAYGO_CART_MAIN_MENU_SLUG', 'rapaygo-menu-main' );

//Loading language files
//Set up localisation. First loaded overrides strings present in later loaded file
$locale = apply_filters( 'plugin_locale', get_locale(), 'rapaygo-wp-plugin' );
load_textdomain( 'rapaygo-wp-plugin', RAPAYGO_LANG_DIR . "/rapaygo-wp-plugin-$locale.mo" );
load_plugin_textdomain( 'rapaygo-wp-plugin', false, RAPAYGO_CART_FOLDER . '/languages' );

include_once('rapaygo-wp-plugin_utility_functions.php');
include_once('rapaygo-wp-plugin_shortcodes.php');
...
if ( wp_doing_ajax() ) {
    add_action( 'wp_ajax_rapaygo_process_pp_smart_checkout', 'rapaygo_process_pp_smart_checkout' );
    add_action( 'wp_ajax_nopriv_rapaygo_process_pp_smart_checkout', 'rapaygo_process_pp_smart_checkout' );
}

function rapaygo_process_pp_smart_checkout() {
    if ( isset( $_POST[ 'rapaygo_payment_data' ] ) ) {
	$data = $_POST[ 'rapaygo_payment_data' ];
    }
    if ( empty( $data ) ) {
	wp_send_json( array( 'success' => false, 'errMsg' => __( 'Empty payment data received.', "rapaygo-wp-plugin" ) ) );
	}
    ...
    $debug_enabled	 = false;
    $debug		 = get_option( 'rapaygo_enable_debug' );
    if ( $debug ) {
	$debug_enabled = true;
    }
	...
	wp_send_json( array( 'success' => false, 'errMsg' => __( 'Error occured during payment processing. Check debug log for additional details.', "rapaygo-wp-plugin" ) ) );
    }
}
...
function reset_rapaygo_cart() {
    ...
    unset( $_SESSION[ 'rapaygo_cart_action_msg' ] );
    unset( $_SESSION[ 'rapaygo_discount_applied_once' ] );
    unset( $_SESSION[ 'rapaygo_applied_coupon_code' ] );
}

function rapaygo_cart_actions_handler() {
	if ( isset( $_SESSION['rapaygo_cart_action_msg'] ) ) {
		unset( $_SESSION['rapaygo_cart_action_msg'] );
	}
	...
	if ( get_option( 'rapaygo_disable_nonce_add_cart' ) ) {
	    ...
	    if ( ! wp_verify_nonce( $nonce, 'rapaygo_addcart' ) ) {
	...
	//Sanitize post data
	$post_rapaygo_product	 = isset( $_POST[ 'rapaygo_product' ] ) ? stripslashes( sanitize_text_field( $_POST[ 'rapaygo_product' ] ) ) : '';
	...
	$post_thumbnail		 = isset( $_POST[ 'thumbnail' ] ) ? esc_url_raw( sanitize_text_field( $_POST[ 'thumbnail' ] ) ) : '';
        $post_rapaygo_tmp_name	 = isset( $_POST[ 'product_tmp' ] ) ? stripslashes( sanitize_text_field( $_POST[ 'product_tmp' ] ) ) : '';

	//Sanitize and validate price
	if ( isset( $_POST[ 'price' ] ) ) {
	    $price		 = sanitize_text_field( $_POST[ 'price' ] );
	    $hash_once_p	 = sanitize_text_field( $_POST[ 'hash_one' ] );
	    $p_key		 = get_option( 'rapaygo_private_key_one' );
	    $hash_one_cm	 = md5( $p_key . '|' . $price . '|' . $post_rapaygo_tmp_name );
	...
	if ( isset( $_POST[ 'shipping' ] ) ) {
	    ...
	    $p_key		 = get_option( 'rapaygo_private_key_one' );
	    $hash_two_cm	 = md5( $p_key . '|' . $shipping . '|' . $post_rapaygo_tmp_name );
	...
	if ( isset( $_SESSION[ 'simpleCart' ] ) ) {
	    ...
	    if ( is_array( $products ) ) {
		foreach ( $products as $key => $item ) {
		    if ( $item[ 'name' ] == $post_rapaygo_product ) {
			$count += $item[ 'quantity' ];
			$item[ 'quantity' ] ++;
			unset( $products[ $key ] );
	...
	if ( $count == 1 ) {
	    ...
	    $product = array( 'name' => $post_rapaygo_product, 'price' => $price, 'price_orig' => $price, 'quantity' => $count, 'shipping' => $shipping, 'cartLink' => $post_cart_link, 'item_number' => $post_item_number );
	...
	rapaygo_reapply_discount_coupon_if_needed(); //Re-apply coupon to the cart if necessary

	if ( ! isset( $_SESSION[ 'simple_cart_id' ] ) && empty( $_SESSION[ 'simple_cart_id' ] ) ) {
	    rapaygo_insert_new_record();
	} else {
	    //cart updating
	    if ( isset( $_SESSION[ 'simple_cart_id' ] ) && ! empty( $_SESSION[ 'simple_cart_id' ] ) ) {
		rapaygo_update_cart_items_record();
	    } else {
		echo "<p>" . (__( "Error! Your session is out of sync. Please reset your session.", "rapaygo-wp-plugin" )) . "</p>";
	    }
	}
	...
	if ( get_option( 'rapaygo_auto_redirect_to_checkout_page' ) ) {
		...
		echo "<br /><strong>" . (__( "Shopping Cart Configuration Error! You must specify a value in the 'Checkout Page URL' field for the automatic redirection feature to work!", "rapaygo-wp-plugin" )) . "</strong><br />";
	    } else {
	...
	$nonce = $_REQUEST[ '_wpnonce' ];
	if ( ! wp_verify_nonce( $nonce, 'rapaygo_cquantity' ) ) {
	    wp_die( 'Error! Nonce Security Check Failed!' );
	}
	$post_rapaygo_product	 = isset( $_POST[ 'rapaygo_product' ] ) ? stripslashes( sanitize_text_field( $_POST[ 'rapaygo_product' ] ) ) : '';
	...
	$products = $_SESSION[ 'simpleCart' ];
	foreach ( $products as $key => $item ) {
	    if ( (stripslashes( $item[ 'name' ] ) == $post_rapaygo_product) && $post_quantity ) {
		$item[ 'quantity' ] = $post_quantity;
		unset( $products[ $key ] );
		array_push( $products, $item );
	    } else if ( ($item[ 'name' ] == $post_rapaygo_product) && ! $post_quantity ) {
		unset( $products[ $key ] );
	    }
	}
	sort( $products );
	...
	$nonce = $_REQUEST[ '_wpnonce' ];
	if ( ! wp_verify_nonce( $nonce, 'rapaygo_delcart' ) ) {
	    wp_die( 'Error! Nonce Security Check Failed!' );
	}
	$post_rapaygo_product	 = isset( $_POST[ 'rapaygo_product' ] ) ? stripslashes( sanitize_text_field( $_POST[ 'rapaygo_product' ] ) ) : '';
	$products		 = $_SESSION[ 'simpleCart' ];
	...
	$nonce = $_REQUEST[ '_wpnonce' ];
	if ( ! wp_verify_nonce( $nonce, 'rapaygo_coupon' ) ) {
	    wp_die( 'Error! Nonce Security Check Failed!' );
	}
	$coupon_code = isset( $_POST[ 'rapaygo_coupon_code' ] ) ? sanitize_text_field( $_POST[ 'rapaygo_coupon_code' ] ) : '';
...
function rapaygo_cart_add_custom_field() {
    $_SESSION[ 'rapaygo_cart_custom_values' ]	 = "";
    $custom_field_val			 = "";
    $name					 = 'rapaygo_cart_id';
    $value					 = $_SESSION[ 'simple_cart_id' ];
    $custom_field_val			 = rapaygo_append_values_to_custom_field( $name, $value );
    ...
    if ( isset( $_SESSION[ 'rapaygo_applied_coupon_code' ] ) ) {
	$name			 = "coupon_code";
	$value			 = $_SESSION[ 'rapaygo_applied_coupon_code' ];
	$custom_field_val	 = rapaygo_append_values_to_custom_field( $name, $value );
    }

    //Trigger action hook that can be used to append more custom fields value that is saved to the session ($_SESSION[ 'rapaygo_cart_custom_values' ])
    do_action('rapaygo_cart_custom_field_appended');

    $custom_field_val	 = apply_filters( 'rapaygo_cart_custom_field_value', $custom_field_val );
...
function print_rapaygo_cart_button_deprecated( $content ) {
    $addcart = get_option( 'addToCartButtonName' );
    if ( ! $addcart || ($addcart == '') )
	$addcart = __( "Add to Cart", "rapaygo-wp-plugin" );
	...
	$replacement	 = '<div class="rapaygo_cart_button_wrapper">';
	$replacement	 .= '<form method="post" class="wp-cart-button-form" action="" style="display:inline" onsubmit="return ReadForm(this, true);" ' . apply_filters( "rapaygo_add_cart_button_form_attr", "" ) . '>';
	$replacement	 .= wp_nonce_field( 'rapaygo_addcart', '_wpnonce', true, false ); //nonce value
	...
	if ( preg_match( "/http/", $addcart ) ) {
	    ...
	    $replacement .= '<input type="image" src="' . $addcart . '" class="rapaygo_cart_button" alt="' . (__( "Add to Cart", "rapaygo-wp-plugin" )) . '"/>';
	} else {
	    //Plain text add to cart button
	    $replacement .= '<input type="submit" class="rapaygo_add_cart_submit" name="rapaygo_add_cart_submit" value="' . $addcart . '" />';
	}

	$replacement	 .= '<input type="hidden" name="rapaygo_product" value="' . $pieces[ '0' ] . '" /><input type="hidden" name="price" value="' . $pieces[ '1' ] . '" />';
	...
	if ( empty( $p_key ) ) {
	    $p_key = uniqid( '', true );
	    update_option( 'rapaygo_private_key_one', $p_key );
	}
...
function rapaygo_cart_add_read_form_javascript() {
    $debug_marker = "<!-- Rapaygo Shopping Cart plugin v" . RAPAYGO_CART_VERSION . " - -->";
    echo "\n${debug_marker}\n";
    echo '
	...
	function ReadForm (obj1, tst)
	{
		...
		obj1.rapaygo_product.value = val_total;
	}
	//-->
	</script>';
}

function print_rapaygo_cart_button_for_product( $name, $price, $shipping = 0, $var1 = '', $var2 = '', $var3 = '', $atts = array() ) {
    $addcart = get_option( 'addToCartButtonName' );
    if ( ! $addcart || ($addcart == '') ) {
	$addcart = __( "Add to Cart", "rapaygo-wp-plugin" );
    }
	...
	$var_output	 .= '<span class="rapaygo_cart_variation_name">' . $variation1_name . ' : </span>';
	$var_output	 .= '<select name="variation1" class="rapaygo_cart_variation1_select" onchange="ReadForm (this.form, false);">';
	...
	$var_output	 .= '<span class="rapaygo_cart_variation_name">' . $variation2_name . ' : </span>';
	$var_output	 .= '<select name="variation2" class="rapaygo_cart_variation2_select" onchange="ReadForm (this.form, false);">';
	...
	$var_output	 .= '<span class="rapaygo_cart_variation_name">' . $variation3_name . ' : </span>';
	$var_output	 .= '<select name="variation3" class="rapaygo_cart_variation3_select" onchange="ReadForm (this.form, false);">';
    ...
    $replacement	 = '<div class="rapaygo_cart_button_wrapper">';
    $replacement	 .= '<form method="post" class="wp-cart-button-form" action="" style="display:inline" onsubmit="return ReadForm(this, true);" ' . apply_filters( "rapaygo_add_cart_button_form_attr", "" ) . '>';
    $replacement	 .= wp_nonce_field( 'rapaygo_addcart', '_wpnonce', true, false );
    if ( ! empty( $var_output ) ) {//Show variation
	$replacement .= '<div class="rapaygo_cart_variation_section">' . $var_output . '</div>';
    }

    if ( isset( $atts[ 'button_image' ] ) && ! empty( $atts[ 'button_image' ] ) ) {
	//Use the custom button image specified in the shortcode
	$replacement .= '<input type="image" src="' . $atts[ 'button_image' ] . '" class="rapaygo_cart_button" alt="' . (__( "Add to Cart", "rapaygo-wp-plugin" )) . '"/>';
    } else if ( isset( $atts[ 'button_text' ] ) && ! empty( $atts[ 'button_text' ] ) ) {
	//Use the custom button text specified in the shortcode
	$replacement .= '<input type="submit" class="rapaygo_add_cart_submit" name="rapaygo_add_cart_submit" value="' . apply_filters( 'rapaygo_add_cart_submit_button_value', $atts[ 'button_text' ], $price ) . '" />';
    } else {
	...
	if ( preg_match( "/http:/", $addcart ) || preg_match( "/https:/", $addcart ) ) {
	    ...
	    $replacement .= '<input type="submit" class="rapaygo_add_cart_submit" name="rapaygo_add_cart_submit" value="' . apply_filters( 'rapaygo_add_cart_submit_button_value', $addcart, $price ) . '" />';
	}
    }

    $replacement	 .= '<input type="hidden" name="rapaygo_product" value="' . $name . '" />';
...
function print_payment_currency( $price, $symbol, $decimal = '.' ) {
    $formatted_price = '';
    $formatted_price = apply_filters( 'rapaygo_print_formatted_price', $formatted_price, $price, $symbol );
...
function simple_cart_total() {
    ...
    return rapaygo_number_format_price( $grand_total );
}

function rapaygo_paypal_shopping_cart_load_widgets() {
    $widget_options = array('classname' => 'rapaygo_paypal_shopping_cart_widgets', 'description' => __("Rapaygo Shopping CartWidget"));
    wp_register_sidebar_widget('rapaygo_paypal_shopping_cart_widgets', __('Rapaygo Shopping Cart'), 'show_rapaygo_cart_widget', $widget_options);
}

function show_rapaygo_cart_widget( $args ){
    // outputs the content of the widget
    extract( $args );

    $cart_title	 = get_option( 'rapaygo_cart_title' );
    if ( empty( $cart_title ) ){
        $cart_title	 = __( "Shopping Cart", "rapaygo-wp-plugin" );
    }
...
function rapaygo_admin_side_enqueue_scripts() {
    if ( isset( $_GET[ 'page' ] ) && $_GET[ 'page' ] == 'rapaygo-discounts' ) { //simple paypal shopping cart discount page
	wp_enqueue_style( 'jquery-ui-style', RAPAYGO_CART_URL . '/assets/jquery.ui.min.css', array(), RAPAYGO_CART_VERSION );

	wp_register_script( 'wpspsc-admin', RAPAYGO_CART_URL . '/lib/rapaygo_admin_side.js', array( 'jquery', 'jquery-ui-datepicker' ) );
	wp_enqueue_script( 'wpspsc-admin' );
    }
}

function rapaygo_admin_side_styles() {
    wp_enqueue_style( 'rapaygo-admin-style', RAPAYGO_CART_URL . '/assets/rapaygo-admin-styles.css', array(), RAPAYGO_CART_VERSION );
}

function rapaygo_front_side_enqueue_scripts() {
    wp_enqueue_style( 'rapaygo-style', RAPAYGO_CART_URL . '/rapaygo_style.css', array(), RAPAYGO_CART_VERSION );
}

function rapaygo_plugin_install() {
    rapaygo_run_activation();
}

register_activation_hook( __FILE__, 'rapaygo_plugin_install' );

// Add the settings link
function rapaygo_cart_add_settings_link( $links, $file ) {
    if ( $file == plugin_basename( __FILE__ ) ) {
	$settings_link = '<a href="admin.php?page=rapaygo-menu-main">' . (__( "Settings", "rapaygo-wp-plugin" )) . '</a>';
	array_unshift( $links, $settings_link );
    }
    return $links;
}

add_filter( 'plugin_action_links', 'rapaygo_cart_add_settings_link', 10, 2 );

add_action( 'widgets_init', 'rapaygo_paypal_shopping_cart_load_widgets' );

add_action( 'init', 'rapaygo_cart_init_handler' );
add_action( 'admin_init', 'rapaygo_cart_admin_init_handler' );

add_filter( 'the_content', 'print_rapaygo_cart_button_deprecated', 11 );
...
add_action( 'wp_head', 'rapaygo_cart_add_read_form_javascript' );
add_action( 'wp_enqueue_scripts', 'rapaygo_front_side_enqueue_scripts' );
add_action( 'admin_enqueue_scripts', 'rapaygo_admin_side_enqueue_scripts' );
add_action( 'admin_print_styles', 'rapaygo_admin_side_styles' );
